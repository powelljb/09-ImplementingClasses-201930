...
class Point(object):
    ...
    def move_to(self,x,y):
        self.total_distance_so_far = self.total_distance_so_far + math.sqrt((self.x-x)**2 + (self.y - y)**2)
        self.x = x
        self.y = y
        self.distance1 = math.sqrt(abs(self.x - self.startx) ** 2 + abs(self.y - self.starty) ** 2)
    ...
    def move_by(self,dx,dy):
        self.total_distance_so_far = self.total_distance_so_far + math.sqrt((self.x - (self.x + dx))**2 + (self.y - (self.y + dy))**2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.distance1 = math.sqrt(abs(self.x - self.startx)**2 + abs(self.y-self.starty)**2)
        self.moves = self.moves + 1
    ...
    def get_distance_from(self,Point):
        x_point = abs(self.x - Point.x)
        y_point = abs(self.y - Point.y)
    ...
    def get_distance_traveled(self):
        return self.total_distance_so_far
    ...
    def halfway_to(self,p2):
        averageX = (self.x + p2.x)/2
        averageY = (self.y + p2.y)/2
        halfway_point = Point(averageX,averageY)
